<div id="{{ str_random(10) }}" class="form-group {{ $errors->has($field_id)?'has-error':'' }}">
    ...
    <div class="form-control-add-rotation text-center {{ $errors->has($field_id)?'has-error':'' }}">
        ...
        <input type="hidden" value="{{ old($field_id) }}" name="{{ $field_id }}" class="rotation-info">

        <div class="rotation-add" @if (old($field_id) != '') style="display:none" @endif>
        ...
        <div class="rotation-edit" @if (old($field_id) == '') style="display:none" @endif>
