<div id="{{ str_random(10) }}" class="form-group {{ $errors->has($field_id)?'has-error':'' }}">
    ...
    <div class="form-control-add-file text-center {{ $errors->has($field_id)?'has-error':'' }}">
        ...
        <input type="hidden" @if (isset($form['#content']['#id'])) value="{{ $form['#content']['#id'] }}" @else value="" @endif name="{{ $field_id }}" class="video-id">

        <div class="video-add" @if (isset($form['#content']['#value'])) style="display:none" @endif>
        ...
        <div class="video-edit" @if (!isset($form['#content']['#value'])) style="display:none" @endif>
            <div class="video-placeholder" style="margin-bottom:10px"><video class="edit img-thumbnail center-block" width="300" height="auto" preload="metadata"><source src="@if (isset($form['#content']['#value'])) {{ $form['#content']['#value'] }} @endif" type="video/mp4"></video></div>
    ...
    <span class="info-block">@if (!is_null($theme['theme-key'])) {!! trans($theme['theme-key'] . '::' . $form['#help']) !!} @else {!! $form['#help'] !!} @endif @foreach ($form['#file-extension'] as $file_ext) <span class="label label-warning">{{ $file_ext }}</span>@endforeach @if ($form['#required'] == true) <span class="label label-danger">{{ trans('template_fields.required') }}</span>@endif</span>
