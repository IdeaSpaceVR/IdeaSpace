<div class="form-group {{ $errors->has($field_id)?'has-error':'' }}">
    ...
    @elseif ($form['#contentformat'] == App\Content\FieldTypeTextarea::CONTENTFORMAT_HTML_TEXT)
        <div id="{{ $field_id }}" name="{{ $field_id }}" class="field-type-textarea form-control-textarea {{ $errors->has($field_id)?'has-error':'' }}" data-placeholder="{{ $form['#description'] }}">{!! old($field_id) !!}</div>
        {!! Form::textarea($field_id, '', array('class'=>'form-control input-lg', 'placeholder'=> $form['#description'], 'rows' => $form['#rows'], 'maxlength' => $form['#maxlength'], 'style' => 'display:none')) !!}
    @endif
    <span class="info-block">{{ $form['#help'] }} @if ($form['#contentformat'] == App\Content\FieldTypeTextarea::CONTENTFORMAT_HTML_TEXT) <span style="color:#000000;font-weight:bold">{{ trans('template_fields.formatted_text_hint') }}</span>@endif @if ($form['#maxlength'] != App\Content\FieldTypeTextarea::DEFAULT_MAXLENGTH) <span class="label label-warning">{{ trans('template_fields.max') }} {{ $form['#maxlength'] }} {{ trans('template_fields.characters') }}</span> @endif <span class="label label-warning">@if ($form['#contentformat'] == App\Content\FieldTypeTextarea::CONTENTFORMAT_HTML_TEXT) {{ trans('template_fields.formatted_text') }}@else{{ trans('template_fields.plain_text') }}@endif</span> @if ($form['#required'] == true) <span class="label label-danger">{{ trans('template_fields.required') }}</span>@endif</span>
