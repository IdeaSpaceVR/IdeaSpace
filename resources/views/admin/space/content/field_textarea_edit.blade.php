<div class="form-group {{ $errors->has($field_id)?'has-error':'' }}">
    ...
    @if ($form['#contentformat'] == App\Content\FieldTypeTextarea::CONTENTFORMAT_TEXT)
        {!! Form::textarea($field_id, $form['#content']['#value'], array('class'=>'form-control input-lg', 'placeholder'=> ((!is_null($theme['theme-key']))?trans($theme['theme-key'] . '::' . $form['#description']):$form['#description']), 'rows' => $form['#rows'], 'maxlength' => $form['#maxlength'])) !!}
    @elseif ($form['#contentformat'] == App\Content\FieldTypeTextarea::CONTENTFORMAT_HTML_TEXT)
				@if (isset($form['#field-group']))
        {!! Form::textarea($field_id, $form['#content']['#value'], array('class'=>'form-control input-lg group-field-type-textarea', 'placeholder'=> ((!is_null($theme['theme-key']))?trans($theme['theme-key'] . '::' . $form['#description']):$form['#description']), 'rows' => $form['#rows'], 'maxlength' => $form['#maxlength'])) !!}
				@else
        {!! Form::textarea($field_id, $form['#content']['#value'], array('class'=>'form-control input-lg field-type-textarea', 'placeholder'=> ((!is_null($theme['theme-key']))?trans($theme['theme-key'] . '::' . $form['#description']):$form['#description']), 'rows' => $form['#rows'], 'maxlength' => $form['#maxlength'])) !!}
				@endif
    @endif
