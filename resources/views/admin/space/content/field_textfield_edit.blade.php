<div class="form-group {{ $errors->has($field_id)?'has-error':'' }}">
    ...
    @if ($form['#contentformat'] == App\Content\FieldTypeTextfield::CONTENTFORMAT_TEXT)
        {!! Form::text($field_id, $form['#content']['#value'], array('class'=>'form-control input-lg', 'placeholder'=> ((!is_null($theme['theme-key']))?trans($theme['theme-key'] . '::' . $form['#description']):$form['#description']), 'maxlength' => $form['#maxlength'])) !!}
    @elseif ($form['#contentformat'] == App\Content\FieldTypeTextfield::CONTENTFORMAT_HTML_TEXT)
				@if (isset($form['#field-group']))
        {!! Form::text($field_id, $form['#content']['#value'], array('class'=>'form-control input-lg group-field-type-textfield', 'placeholder'=> ((!is_null($theme['theme-key']))?trans($theme['theme-key'] . '::' . $form['#description']):$form['#description']), 'maxlength' => $form['#maxlength'])) !!}
				@else
        {!! Form::text($field_id, $form['#content']['#value'], array('class'=>'form-control input-lg field-type-textfield', 'placeholder'=> ((!is_null($theme['theme-key']))?trans($theme['theme-key'] . '::' . $form['#description']):$form['#description']), 'maxlength' => $form['#maxlength'])) !!}
				@endif
    @endif
