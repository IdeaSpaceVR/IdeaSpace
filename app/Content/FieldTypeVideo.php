...
class FieldTypeVideo {
    ...
    private $template_add_script = 'public/assets/admin/space/content/js/field_video_add.js';
    private $template_edit_script = 'public/assets/admin/space/content/js/field_video_edit.js';
    ...
    public function get_validation_rules_messages($request, $validation_rules_messages, $field_key, $properties) {
        ...
        if ($request->input($field_key) != '' ) {
            ...
            $video = Video::where('id', $field_value)->first();
            $genericFile = GenericFile::where('id', $video->file_id)->first();
        ...
        if ($properties['#required']) {
            ...
            $validation_rules_messages['messages'] = array_dot(array_add(
                ...
                $field_key . '.required',
                trans('fieldtype_video.validation_required', ['label' => $properties['#label']])
            ));
            ...
            $validation_rules_messages['messages'] = array_dot(array_add(
                $validation_rules_messages['messages'],
                $field_key . '.in',
                trans('fieldtype_video.validation_in', ['label' => $properties['#label'], 'file_extensions' => implode(', ', explode(',', $file_extensions))])
            ));
    ...
    public function save($content_id, $field_key, $type, $request_all) {
        ...
        try {
            ...
            if (array_has($request_all, $field_key . '__video_id')) {
                $field->value = $request_all[$field_key . '__video_id'];
                $field->save();
            } else {
                $field->delete();
            }
