...
class FieldTypePhotosphere {
    ...
    public function load($content_id, $field_key, $properties) {
        ...
        try {
            $photosphere = Photosphere::where('id', $field->value)->firstOrFail();
        } catch (ModelNotFoundException $e) {
            return $field_arr;
        }
        ...
        $field_arr['#content']['#id'] = $photosphere->id;
    ...
    public function get_validation_rules_messages($request, $validation_rules_messages, $field_key, $properties) {
        ...
        if ($request->input($field_key) != '' ) {
            ...
            $photosphere = Photosphere::where('id', $field_value)->first();
            $genericFile = GenericFile::where('id', $photosphere->file_id)->first();
            ...
            $request->request->add([$field_key . '__photosphere_id' => $field_value]);
            ...
            $request->request->add([$field_key . '__photosphere_src' => asset($genericFileThumbnail)]);
        }
        ...
        if ($properties['#required']) {
            ...
            $validation_rules_messages['messages'] = array_dot(array_add(
                ...
                $field_key . '.required',
                trans('fieldtype_photosphere.validation_required', ['label' => $properties['#label']])
            ));
            ...
            $validation_rules_messages['messages'] = array_dot(array_add(
                $validation_rules_messages['messages'],
                $field_key . '.in',
                trans('fieldtype_photosphere.validation_in', ['label' => $properties['#label'], 'file_extensions' => implode(', ', explode(',', $file_extensions))])
            ));
    ...
    public function save($content_id, $field_key, $type, $request_all) {
        ...
        try {
            ...
            if (array_has($request_all, $field_key . '__photosphere_id')) {
                $field->value = $request_all[$field_key . '__photosphere_id'];
                $field->save();
            } else {
                $field->delete();
            }
